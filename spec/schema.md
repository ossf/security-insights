...
A URI identifying the attestation’s predicate type. Example: `https://slsa.dev/provenance/v1`
...
Indicates whether this is the first point of contact for inquiries. Only one entry should be marked as primary.
...
The canonical URL where this file is published (e.g., a raw file URL in a version control system). It lets readers find the latest version of the file when they hold an outdated copy, such as one from a past release artifact. This is not the project URL — list project URLs in `project.repositories` instead.
...
The URL of a parent Security Insights file whose project information this file inherits. Use this when many repositories share one project's data. The URL must be publicly readable and return raw YAML (content-type "text/plain" or "application/yaml"), not an HTML page.

## #License

License identifies a license by its SPDX expression and a link to its text.

`expression` **string** _Required_

The license in [SPDX expression](https://spdx.org/licenses/) format. Example: `Apache-2.0`
...
Link is a URI paired with a comment explaining what it points to.
...
An organization that provides sustained support for the development of this project as free and open-source software — a "steward" as defined by the [European Union Cyber Resilience Act, Article 3](https://eur-lex.europa.eu/eli/reg/2024/2847/oj/eng#art_3).
...
Indicates whether the tool is used in a scheduled process or supports on-demand execution.
...
URL is a web URL beginning with http:// or https://
